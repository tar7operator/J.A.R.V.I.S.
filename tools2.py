...
def wiki(something):
    ...
    results = wikipedia.summary(something, sentences=2)
...
def set_remainder(something):
    ...
    if "add" in something:
        ...
        writer.writerow({'time': r_time, 'date': date, 'message': message})
        speak("Reminder added successfully!")

    elif "change" in something or "update" in something:
        if "message" in something:
            pass
        elif "time" in something:
            pass
    else:
        speak("I am sorry, i forgot wat to do.")
